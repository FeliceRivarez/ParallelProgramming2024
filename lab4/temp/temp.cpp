...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        while (testset >> a >> b)
        {
            ...
            int remainder = b.length() % size;
            for (int i = 0; i < size; ++i)
            {
                int start = i * segment_length + min(i, remainder);
                int end = start + segment_length + (i < remainder ? 1 : 0);
                string segment = b.substr(start, end - start);

                int seg_len = segment.length();
                MPI_Send(&seg_len, 1, MPI_INT, i, 0, MPI_COMM_WORLD);

                MPI_Send(segment.c_str(), seg_len, MPI_CHAR, i, 1, MPI_COMM_WORLD);
            }
        }
        end_flag = 1;
        MPI_Bcast(&end_flag, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    else
    {
        ...
        while (!end_flag)
        {
            ...
            char *segment = new char[seg_length + 1];
            MPI_Recv(segment, seg_length, MPI_CHAR, 0, 1, MPI_COMM_WORLD, &status);
            segment[seg_length] = '\0'; // Null-terminate the string

            cout << segment << endl;
            delete[] segment; // Free the allocated memory
        }
    }
    MPI_Finalize();
    // while (testset >> a)
