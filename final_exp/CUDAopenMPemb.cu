// nvcc -o CUDAopenMPemb CUDAopenMPemb.cu -ccbin "C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Tools\MSVC\14.39.33519\bin\Hostx64\x64\cl.exe"
#include <iostream>
#include <queue>
#include <fstream>
#include <chrono>
    ...
    while (testset >> a)
    {
        ...
        for (int k = 0; k < test; k += 1)
        {
            ...
            for (int i = 1; i < m + 1; i += 1)
            {
                ...
                {
                    int i1,i2;
                    int upper=min(i+1+10,m+1);
                    #pragma omp parallel num_threads(NUM_THREADS), private(i2, i1), default(shared)
                    #pragma omp for
                    for (int i2 = i+1; i2 < upper; i2 += 1)
                    {
                        int nearest = 0;
                        for (int i1 = 1; i1 < n + 1; i1 += 1)
                        {
